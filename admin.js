...
$('momentForm').addEventListener('submit',async e=>{
  ...
  try{
    const now=Date.now();
    const moment={title,target:Math.round(target),intro:$('intro').value.trim(),theme:$('theme').value,durationHours,startsAt:now,endsAt:now+durationHours*36e5,finalText,confirmPost:$('confirmPost').checked,allowWithdrawal:$('allowWithdrawal').checked,confirmEdit:$('confirmEdit').checked,confirmDelete:$('confirmDelete').checked,allowShare:$('allowShare').checked,animationType:$('animationType').value,gaugeVisibility:$('gaugeVisibility').value,forceFinal:false,archived:false,createdAt:now};
    ...
    e.target.reset();$('finalText').value=$('finalPreset').value;$('allowShare').checked=true;
...
function momentCard(id,m){
  ...
  const shareState=m.allowShare===false?'🔒 Partage désactivé':'🔗 Partage autorisé';
  const card=document.createElement('article');card.className='moment-admin dashboard-card';
  card.innerHTML=`
    <div class="moment-top"><div><span class="status-pill ${stateClass}">${state}</span><h3>${escapeHtml(m.title||'Sans titre')}</h3><p class="muted">${escapeHtml(m.theme||'Libre')} · ${animationLabels[m.animationType]||animationLabels.cauldron} · ${shareState}</p></div><div class="metric"><strong>${count}/${m.target||1}</strong><span>participations</span></div></div>
    <div class="progress"><div style="width:${pct}%"></div></div>
    <div class="moment-meta"><span>${pct}%</span><span>Fin : ${fmt(m.endsAt)}</span></div>
    <div class="action-grid primary-actions"><button class="primary copy">📋 Copier le lien</button><a class="secondary" target="_blank" href="${momentUrl(id)}">👁 Ouvrir</a><button class="secondary messages">💬 Voir les messages</button><button class="launch">🔥 Lancer la cérémonie</button></div>
  ...
  card.querySelector('.edit').onclick=()=>editMoment(id,m);card.querySelector('.messages').onclick=()=>showMessages(id,m);card.querySelector('.preview').onclick=()=>window.open(momentUrl(id,true),'_blank');card.querySelector('.launch').onclick=()=>launchCeremony(id,m);card.querySelector('.closeMoment').onclick=()=>closeMoment(id,m);card.querySelector('.reopen').onclick=()=>reopenMoment(id,m);card.querySelector('.archive').onclick=()=>update(ref(db,`moments/${id}`),{archived:!m.archived,archivedAt:!m.archived?Date.now():null});card.querySelector('.reset').onclick=()=>resetMoment(id,m);card.querySelector('.delete').onclick=()=>deleteMoment(id,m);
